...
export const FormValidator: React.FC<FormValidatorProps> = ({ value, rules, className = '' }) => {
  ...
  return (
    <div className={`mt-2 space-y-1 ${className}`}>
      {results.map((result, index) => {
        ...
        return (
          <div key={index} className={`flex items-center space-x-2 text-sm ${colorClass}`}>
            <Icon size={14} />
...
export const ValidationRules = {
  ...
  phone: {
    test: (value: string) => /^\+\d{1,4}\s\d{2}\s\d{2}\s\d{2}\s\d{2}$/.test(value),
  ...
  passwordStrong: {
    ...
    message: 'Mot de passe fort (8+ caractères, majuscule, minuscule, chiffre, caractère spécial)',
    severity: 'error' as const
  },
  
  passwordMedium: {
    test: (value: string) => /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[A-Za-z\d@$!%*?&]{6,}$/.test(value),
    message: 'Mot de passe moyen (6+ caractères, majuscule, minuscule, chiffre)',
